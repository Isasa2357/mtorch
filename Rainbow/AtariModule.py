...
from ..util.nnModuleFactory import factory_LinearReLU_Sequential
from ..Network.Qnet import BaseQnetwork
from ..Network.NoisyNet import NoisyNetInterface
from ..layer.NoisyLinear import NoisyLinear
...
class AtariQnetwork(BaseQnetwork):
    ...
    画像を入力とするQnetwork
    '''
    def __init__(self, frame_size: Tuple[int, int], frame_num: int, out_chnls: int, dueling: bool=False):
        super().__init__()
...
class AtariNoisyQnetwork(NoisyNetInterface):
    '''
    画像を入力とするQnetwork
    '''
    def __init__(self, frame_size: Tuple[int, int], frame_num: int, out_chnls: int, dueling: bool=False):
        ...
        self._fe = NoisyLinear(64*7*7, 512)
        self._fe_relu = nn.ReLU()
        self._head = NoisyLinear(512, self._out_chnls)
        self._base_head = NoisyLinear(512, 1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self._dueling:
            ...
            base = self._base_head.forward(x)
            advantage = self._head.forward(x)
            x = base + advantage
